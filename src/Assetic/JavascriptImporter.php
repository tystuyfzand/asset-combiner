...
class JavascriptImporter implements FilterInterface {
    ...
    protected function directiveInclude($data, $required = false) {
        ...
        foreach ($require as $script) {
            ...
            $bowerScriptPath = realpath($this->scriptPath . '/../bower/' . $script);

            if (!File::isFile($scriptPath) && !File::isFile($bowerScriptPath)) {
            ...
            } else if (File::isFile($bowerScriptPath)) {
                $scriptPath = $bowerScriptPath;
            }
